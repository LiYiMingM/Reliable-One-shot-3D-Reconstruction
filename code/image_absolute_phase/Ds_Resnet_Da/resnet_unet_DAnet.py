...
class Resnet_Unet(nn.Module):
    ...
    def __init__(self, backbone='resnet34',input_channel=1,out_channels_final=1,BN_enable=True, resnet_pretrain=True):
        ...
        # decoder部分
        self.center = DecoderBlock(in_channels=filters[3], mid_channels=filters[3]*4, out_channels=filters[3], BN_enable=self.BN_enable)#通道数没变，长采样一次
    ...
    def forward(self,x):
        ...
        # print('sasc_output[0].shape',sasc_output[0].shape)
...
class WHOLE_NET(nn.Module):
    def __init__(self,backbone='resnet34'):
        ...
        self.phl3= Resnet_Unet(input_channel=1, out_channels_final=1,backbone=self.backbone)
        self.wrapped = Resnet_Unet(input_channel=2, out_channels_final=1,backbone=self.backbone)
        self.unwrapped = Resnet_Unet(input_channel=3, out_channels_final=1,backbone=self.backbone)
